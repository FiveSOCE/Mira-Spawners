...
public final class MiraSpawnersPlugin extends JavaPlugin {
    ...
    public void onDisable() {
        if (core != null) {
            if (api != null) {
                core.services().unregister(MiraSpawnersApi.class, api);
            }
    ...
    public int maxSpawnerStack() {
        return Math.max(1, Math.min(HARD_MAX_SPAWNER_STACK,
                getConfig().getInt("spawners.max-stack-size", HARD_MAX_SPAWNER_STACK)));
    }

    public boolean silkTouchRequired() {
        return getConfig().getBoolean("spawners.silk-touch-required", true);
    }

    public boolean naturalSpawnersHarvestable() {
        return getConfig().getBoolean("spawners.natural-spawners-harvestable", true);
    }

    public boolean protectSpawnersFromExplosions() {
        return getConfig().getBoolean("spawners.protect-from-explosions", true);
    }

    public int maxMobStack() {
        return Math.max(1, getConfig().getInt("mobs.max-stack-size", 1000));
    }

    public double mergeRadius() {
        return Math.max(0.5D, getConfig().getDouble("mobs.merge-radius", 6.0D));
    }

    public boolean showMobStackName() {
        return getConfig().getBoolean("mobs.show-stack-name", true);
    }

    public boolean lavaStackKill() {
        return getConfig().getBoolean("mobs.lava-stack-kill", true);
    }
