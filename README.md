...
[**Download MiraSpawners v0.1.1**](https://github.com/FiveSOCE/Mira-Spawners/releases/download/v0.1.1/MiraSpawners-0.1.1.jar)
...
- Broken spawner items have no custom display name and no lore.
- MiraSpawners no longer writes block-state metadata to new spawner items, avoiding the modern client warning tooltip associated with special block-entity item data.
...
## Vanilla-looking spawner items

MiraSpawners spawner items are intentionally kept visually simple.

They look like regular Minecraft spawner items:

- item name: normal `Spawner`
- no Mira display name
- no custom lore
- no visible stack-size metadata
- no block-entity warning tooltip generated by MiraSpawners
...
Spawner items of the same mob type naturally stack in the player's inventory up to Minecraft's normal stack limit of 64. Different mob types do not merge because their hidden mob-type data differs.
...
Those items are normal-looking spawners and can naturally occupy an inventory stack of 54. They are no longer represented by one custom item carrying an internal `Stack Size: 54` value.
...
You should receive a normal-looking stack of 54 spawner items with no custom lore or custom display name. Place one, right-click the remaining 53 onto it, then left-click the placed spawner and verify the chat reports `54/64`.

Break that stack with Silk Touch and verify 54 spawner item units are returned.
...
In v0.1.1 that API call returns an ItemStack containing 64 actual normal-looking Zombie spawner item units.

## Persistence and safety

- Spawner block stack size is stored in the spawner TileState PersistentDataContainer.
- New spawner items store only the hidden mob type needed by MiraSpawners.
...
```text
build/libs/MiraSpawners-0.1.1.jar
```
